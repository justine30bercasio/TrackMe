const _expenseVerbs = [
  'paid', 'pay', 'spent', 'spend', 'spends', 'spending', 'bought', 'buy', 'buys', 'purchased', 'purchase',
  'cost', 'costs', 'billed', 'charge', 'charged', 'bayad', 'bayaran', 'gastos', 'gumastos', 'namili', 'bili',
  'bumili', 'kumain', 'kain', 'ubos', 'ginastos', 'swipe', 'settle', 'refill',
];

const _incomeVerbs = [
  'salary', 'sweldo', 'sahod', 'earned', 'earn', 'receives', 'receive', 'received', 'bonus', 'allowance',
  'commission', 'payout', 'sold', 'sell', 'benta', 'refund', 'profit', 'income', 'kita', 'prize', 'won',
];
...
class AssistantService {
  ...
  AssistantIntent respond(String rawMessage) {
    ...
    if (message.isEmpty) {
      return const AssistantIntent(kind: 'text', reply: 'Hi! Tell me something like "I paid 100 pesos for food" and I will log it for you.');
    }
    ...
    if (_isGreeting(lower)) {
      return AssistantIntent(
        kind: 'text',
        reply: 'Hi! I\'m your TrackMe assistant. Just tell me what you paid or received — e.g. "I paid 100 pesos for food" — and I\'ll add it to your expenses automatically. Type "help" to see what I can do.',
      );
    }
    ...
    if (_isSalaryLoanQuestion(lower)) {
      return AssistantIntent(
        kind: 'loan',
        reply: 'Your salary day is set to the ${_ordinal(_salaryDay)} of the month in Profile. Loan repayments are scheduled right on that day so you always have funds on payday.',
      );
    }
    ...
    if (_isSalaryInfo(lower)) {
      return AssistantIntent(
        kind: 'text',
        reply: 'Your salary day is the ${_ordinal(_salaryDay)} of each month. I use it to schedule loan repayments and remind you about payday. You can change it in Settings > Profile.',
      );
    }
    ...
    if (amount == null) {
      if (type == null) {
        return const AssistantIntent(
          kind: 'text',
          reply: 'I didn\'t catch a transaction there. Tell me something like:\n\n• "I paid 100 pesos for food"\n• "I received my salary 25000"\n\nType "help" to see more examples.',
        );
      }
      return AssistantIntent(
        kind: type,
        reply: 'I see you want to log as ${type == 'expense' ? 'an expense' : 'income'}, but I couldn\'t find an amount. Example: "I paid 100 pesos for food".',
      );
    }
    ...
    if (type == null) {
      return AssistantIntent(
        kind: 'text',
        reply: 'I found ₱${amount.toStringAsFixed(2)}, but I couldn\'t tell if it\'s an expense or income. Try "I paid X for Y" or "I received X".',
      );
    }
    ...
    final currencyNote = payment != null ? ' via ${_paymentLabel(payment)}' : '';
    ...
    if (!_hasStrongSignal(lower, amount)) {
      return AssistantIntent(
        ...
        autoPost: false,
        draft: draft,
        reply: 'Should I log ${type == 'expense' ? 'an expense of' : 'income of'} ₱${amount.toStringAsFixed(2)}'
    ...
    return AssistantIntent(
      ...
      autoPost: true,
      draft: draft,
      reply: '$emoji Done! I logged ${type == 'expense' ? 'an expense of' : 'income of'} '
  ...
  bool _hasStrongSignal(String lower, double amount) {
    if (_expenseVerbs.any(lower.contains) || _incomeVerbs.any(lower.contains)) return true;
    if (_moneyAnchors.any(lower.contains)) return true;
    return false;
  }

  bool _isGreeting(String lower) {
    return RegExp(r'\b(hi+|hello+|hey+|good\s*(morning|afternoon|evening)|kumusta|musta|kumusta)\b').hasMatch(lower) &&
        lower.length < 60;
  }

  bool _isHelp(String lower) {
    return RegExp(r'\b(help|commands|what can you do|anong kaya mo|how to use|manual)\b').hasMatch(lower);
  }

  bool _isSalaryLoanQuestion(String lower) {
    final salaryRelated = lower.split(' ').any((w) => ['salary', 'sweldo', 'sahod', 'payday', 'due', 'emi'].contains(w));
    final loanRelated = ['loan', 'utang', 'repayment', 'bayad ng loan', 'amortization'].any(lower.contains);
    return salaryRelated && loanRelated;
  }

  bool _isSalaryInfo(String lower) {
    return RegExp(r'\b(salary day|payday|sweldo|sahod|when.*salary|kelan.*sahod)\b').hasMatch(lower);
  }
  ...
  String _detectType(String lower) {
    ...
    if (RegExp(r'paid|gastos|bayad|namili|kumain|bili').hasMatch(lower)) return 'expense';
    return 'text';
  }

  String _detectCategory(String lower) {
    const map = [
      (keys: ['food', 'lunch', 'dinner', 'breakfast', 'merienda', 'snack', 'kain', 'kumain', 'coffee', 'cafe', 'restaurant', 'grocery', 'groceries', 'mango'], cat: 'Food'),
      (keys: ['transport', 'transpo', 'fare', 'gas', 'fuel', 'grab', 'taxi', 'jeep', 'bus', 'train', 'toll', 'parking'], cat: 'Transportation'),
      (keys: ['bills', 'bill', 'electric', 'kuryente', 'water', 'tubig', 'internet', 'wifi', 'broadband', 'phone', 'load', 'rent', 'utility'], cat: 'Bills'),
      (keys: ['shopping', 'shirt', 'clothes', 'clothing', 'mall', 'store', 'gadget', 'shoe', 'shoes'], cat: 'Shopping'),
      (keys: ['movie', 'cinema', 'concert', 'netflix', 'spotify', 'game', 'gaming', 'music'], cat: 'Entertainment'),
      (keys: ['medicine', 'gamot', 'doctor', 'dentist', 'hospital', 'clinic', 'pharmacy', 'medical', 'checkup'], cat: 'Healthcare'),
      (keys: ['school', 'tuition', 'books', 'classes', 'course'], cat: 'Education'),
      (keys: ['salary', 'sweldo', 'sahod', 'bonus', 'allowance', 'income', 'commission'], cat: 'Income'),
    ];
  ...
  double? _findAmount(String message) {
    ...
    for (final m in bare.allMatches(message)) {
      ...
      final beforeWord = start > 0 ? message.substring(0, start).split(RegExp(r'\s+')).lastOrNull?.toLowerCase() ?? '' : '';
      final v = _parseNumber(m.group(1));
      if (v == null || v <= 0) continue;
      candidates.add(v);
      if (['about', 'around', 'approx', 'approximately', 'roughly', 'nearly', 'almost', 'was', 'is', 'the'].contains(beforeWord)) {
        return v;
      }
    }
  ...
  String _buildDescription(String lower, String type, String category) {
    ...
    for (final v in _expenseVerbs) {
      ...
      if (match != null) {
        final after = lower.substring(match.end).trim();
        final frag = after.split(RegExp(r' (?:via|through|using|ng|na) ')).first;
        final cleaned = frag
            .replaceAll(RegExp(r'[0-9,.\s]+(?:pesos?|pesso|piso|php)?'), ' ')
            .replaceAll(RegExp(r'(?:pesos?|pesso|piso|php|about|for|the|a)'), ' ')
            .replaceAll(RegExp(r'\s+'), ' ')
            .trim();
        if (cleaned.isNotEmpty) return _titleCase(_clamp(cleaned, 24));
      }
